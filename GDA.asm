...
LJMP MAIN
ORG 000BH		;TIMER0 SPACE IN INTERRUPT VECTOR TABLE
LJMP ISR_T0
...
A_i EQU 1
B_i EQU 5
MOV 40H, #LOW A_1		; TRUE VAL OF a
MOV 41H, #LOW B_1		; TRUE VAL OF b
MOV 42H, #LOW A_i		; RANDOM VAL OF a_1
MOV 43H, #LOW B_i		; RANDOM VAL OF b_1

MAIN: MOV TMOD, #01H	;TIMER0, MODE1
MOV DPTR, #65467
MOV TH0, DPH
MOV TL0, DPL
MOV IE, #82H		; IE = 1000 0010 (ENABLE TIMER 0)
MOV R7, #4		; OUTER LOOP FOR i=1000 TIMES
SETB TR0
BACK1: MOV R6, #250	; INNER LOOP FOR i=1000
BACK2: CLR A
ALGORITHM: MOV A, 42H
JB ACC.3, SIGN_a
ANL A, #0FH
MOV 42H, A		; RETURN FIXED a_i TO ITS REG.
SJMP CHECK_b
SIGN_a: ORL A, #0F0H
MOV 42H, A
CHECK_b: MOV A, 43H
JB ACC.3, SIGN_B
ANL A, #0FH
MOV 43H, A		; RETURN FIXED a_i TO ITS REG.
SJMP CONT_ALG		; CONTINUE THE ALGORITHM
SIGN_B: ORL A, #0F0H
MOV 43H, A
CONT_ALG:CLR A
REPEAT:MOV A,R0	; GET BACK THE UPDATED INDEX(i+1)
MOV DPTR, #Xi_LIST
MOVC A, @A+DPTR		; A= Xi
JB ACC.3, SIGN_Xi
ANL A, #0FH
MOV 42H, A
MOV R5, A		; RETURN FIXED a_i TO ITS REG.
SJMP CALCULATE
SIGN_Xi: ORL A, #0F0H
MOV R5, A
CALCULATE:MOV B, 41H	; B =b (TRUE VAL OF b)
MUL AB
ADD A,40H
MOV R2, A		; R2 HAS THE TRUE VALUE OF Y
MOV A, R5		; A = Xi (BRING BACK Xi)
MOV B, 43H
MUL AB
ADD A, 42H		; A = (Y^)_i
CPL A
ADD A, #01H		; 2'S COMLEMENT OF (Y^)_i
MOV R3, A		; R3 = - (Y^)_i
MOV A, R2 
ADD A, R3		; A = []Y_i - (Y^)_i]
PUSH ACC		; SAVE VALUE OF ACC FOR UPDATING b_i PART OF THE ALGORITHM
A_CHECK:CJNE A, #0, UPDATE_a
SJMP B_CHECK		; a_i+1 = a_i !!
UPDATE_a: JC SUBTRACT
MOV A, #01H			; A =1 
ADD A, 42H			; A = a_i +1
MOV 42H, A
SJMP B_CHECK
SUBTRACT: MOV A, #0FFH		; A= -1
ADD A, 42H			;A = a_i -1
MOV 42H, A
B_CHECK: MOV B,R5		; B = Xi (BRING BACK Xi)
POP ACC
MUL AB
CONTINUE: CJNE A, #0, UPDATE_b
SJMP FINISH
UPDATE_b: JC SUBTRACT_2
MOV A, #01H
ADD A, 43H			; A = b_i +1
MOV 43H, A
SUBTRACT_2:  MOV A, #0FFH		; A= -1
ADD A, 43H			;A = b_i -1
MOV 43H, A
FINISH: MOV A, R0		; GET INDEX (i) BACK AND UPDATE IT
INC A
MOV R0, A
DJNZ R6, BACK2
DJNZ R7, BACK1
...
MOV A, #32H	; B= 2
LCALL SEND_DATA
MOV A, #0C0H 	; MOVE TO SECONED LINE OF LCD SCREEN
LCALL SEND_COMMAND
MOV A,#'A'
LCALL SEND_DATA
MOV A, #'='
...
MOV A, #20H	; SPACE IN ASCII
LCALL SEND_DATA
CONTIN:MOV A, #20H	; SPACE IN ASCII
LCALL SEND_DATA
MOV A,#'B'
LCALL SEND_DATA
MOV A, #'='
